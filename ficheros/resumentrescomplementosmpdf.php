...
$cabecera.='<tr><td style="width: 80%; text-align: center;"><h1 class="subtitulo" style="font-size:13px;">Estadística de Items</h1></td></tr></tr></table>';
...
$escribe.='<th class="ceroth">'.$titulo.'<br><br><span class="textonormal">(Nº de alumnos: '.$ii.')</span></th>';
...
// Estadísticas y equipo educativo
// $valoracion="Valoración de la clase [-100%,+100%]= ".$valoracion;

$escribe.='<p class="textonormal"><strong>Equipo educativo: </strong>'.$profesor.'</p>';
...
$mpdf->SetHTMLFooter('<p class="footer">Página  {PAGENO}/{nbpg} - '.$horafecha.' - Leyenda matemática: Nº alumnos - % alumnos calificados con el item - %prevalencia</p>');

// $mpdf->WriteHTML(utf8_encode($escribe));
include_once('watermark.php');
$mpdf->WriteHTML($escribe);
$mpdf->Output('../ficheros/estadisticascomplementos.pdf','F');
// $path='../temporal/A.pdf';
// $mpdf->Output($path,'F');
header("Location: ../ficheros/descargaficheropdf.php?fichero=estadisticascomplementos.pdf");
exit;
