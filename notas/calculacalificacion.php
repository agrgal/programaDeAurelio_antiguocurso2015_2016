...
// ************************
   ...
   $abreviaturas=array_values($abreviaturas); // claves en números correlativas
   $nombreiev=array_values($nombreiev); // claves en números correlativas
   $porcentaje=array_values($porcentaje);
   $notaminima=array_values($notaminima);
   $iev = array_values($iev); // IMPORTANTE: reinicia las claves...
   $sumanotas=array_values($sumanotas);
   $pesonotas=array_values($pesonotas);
   $ponnota=array_values($ponnota);
   ...
   // $tester.=$sumanotas[10]." & ". $ponnota[10]." & ". $pesonotas[10]." & ".$porcentaje[3];

   $datos_json[]='"tester":"'. $tester.'"'; // Enviar la nota total datos.notatotal
   ...
   if(!empty($idcalificacion) && !is_null($idcalificacion)) {
     ...
     // si no existe la nota, se le pone la misma a la nota séneca que a la misma
